...
import com.example.pintbackend.global.exception.InvalidSearchKeywordException;
...
public class UserService {
    ...
    public String getProfileImg(CustomUserDetails userDetails) {
        ...
        return resolveProfileImageUrl(profileImageKey);
    }
    ...
    public MyProfileResponse getProfile(Long targetUserid, CustomUserDetails userDetails) {
        ...
        String profileImageUrl = resolveProfileImageUrl(user.getProfileImageS3Key());

        // target user 포스트 불러오기
        List<ProfilePostResponse> postList = user.getPosts().stream()
                .map(post -> ProfilePostResponse.from(
                        post,
                        s3Service.getPresignedUrlToRead(post.getCompressedImageFileS3Key() != null
                                ? post.getCompressedImageFileS3Key() : post.getImageFileS3Key())
                ))
                .toList();
    ...
    public EditProfileResponse editProfile(Long targetUserId, CustomUserDetails userDetails,
        ...
        user.update(request.getUsername(), request.getIntroduction(), request.getCity(),
                profileImageS3Key);
    ...
    public List<SearchUser> getUserByQuery(String keyword) {
        if (!StringUtils.hasText(keyword)) {
            throw new InvalidSearchKeywordException();
        }

        String normalizedKeyword = normalizeKeyword(keyword);
        List<User> result = userRepository.findByUsernameContainingIgnoreCase(normalizedKeyword);

        return result.stream().map((user ->
                        SearchUser.from(user, resolveProfileImageUrl(user.getProfileImageS3Key()))
                ))
                .toList();
    }
    ...
    private String resolveProfileImageUrl(String profileImageKey) {
        ...
        return s3Service.getPresignedUrlToRead(profileImageKey);
    }
